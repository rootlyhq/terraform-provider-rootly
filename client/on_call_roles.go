...
type OnCallRole struct {
	...
	AlertFieldsPermissions           []interface{} `jsonapi:"attr,alert_fields_permissions,omitempty"`
	AlertGroupsPermissions           []interface{} `jsonapi:"attr,alert_groups_permissions,omitempty"`
	AlertRoutingRulesPermissions     []interface{} `jsonapi:"attr,alert_routing_rules_permissions,omitempty"`
	OnCallReadinessReportPermissions []interface{} `jsonapi:"attr,on_call_readiness_report_permissions,omitempty"`
	OnCallRolesPermissions           []interface{} `jsonapi:"attr,on_call_roles_permissions,omitempty"`
...
func (c *Client) GetOnCallRole(id string) (*OnCallRole, error) {
	req, err := rootlygo.NewGetOnCallRoleRequest(c.Rootly.Server, rootlygo.ID(id))
	if err != nil {
		return nil, fmt.Errorf("Error building request: %w", err)
	}
...
func (c *Client) UpdateOnCallRole(id string, on_call_role *OnCallRole) (*OnCallRole, error) {
	...
	req, err := rootlygo.NewUpdateOnCallRoleRequestWithBody(c.Rootly.Server, rootlygo.ID(id), c.ContentType, buffer)
	if err != nil {
		return nil, fmt.Errorf("Error building request: %w", err)
	}
	resp, err := c.Do(req)
...
func (c *Client) DeleteOnCallRole(id string) error {
	req, err := rootlygo.NewDeleteOnCallRoleRequest(c.Rootly.Server, rootlygo.ID(id))
	if err != nil {
