// Hand-maintained: escalation_level is excluded from client codegen (see tools/generate.js)
// because Delay must be a nullable *int. A plain int cannot distinguish "caller never set
// delay" from "caller explicitly set delay to 0", and both cases need distinct wire behaviour:
// the former must be omitted from the payload, the latter must be sent as 0.
...
type EscalationLevel struct {
	...
	PagingStrategyConfigurationRepeats          int           `jsonapi:"attr,paging_strategy_configuration_repeats,omitempty"`
