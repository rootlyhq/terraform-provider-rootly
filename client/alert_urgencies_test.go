...
func marshalAlertUrgencyAttrs(t *testing.T, p *AlertUrgency) map[string]interface{} {
	t.Helper()
...
func TestAlertUrgencyRetriggerTimeoutMarshal(t *testing.T) {
	// nil pointer (cleared / not configured): the attribute must be present as
	// JSON null so the API resets it to inherit, NOT omitted.
	attrs := marshalAlertUrgencyAttrs(t, &AlertUrgency{ID: "au1", Name: "renamed"})
	...
	never := -1
	attrs = marshalAlertUrgencyAttrs(t, &AlertUrgency{ID: "au1", RetriggerTimeoutMinutes: &never})
	...
	thirty := 30
	attrs = marshalAlertUrgencyAttrs(t, &AlertUrgency{ID: "au1", RetriggerTimeoutMinutes: &thirty})
	if v := attrs["retrigger_timeout_minutes"]; v != float64(30) {
		t.Fatalf("expected 30, got %v", v)
	}
}

func TestAlertUrgencyRetriggerTimeoutUnmarshalNull(t *testing.T) {
	body := `{"data":{"id":"au1","type":"alert_urgencies","attributes":{"name":"n","retrigger_timeout_minutes":null}}}`
	p := new(AlertUrgency)
