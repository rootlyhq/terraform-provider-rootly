...
if (!templatesPath) {
  console.error(
    "No templates JSON specified. Use `node tools/generate-examples.js <templates_file_path>`"
  );
  process.exit(1);
}
...
function genExample(tpl) {
  const params = Object.keys(tpl.config).filter(
    (key) => ["kind", "name", "description"].indexOf(key) === -1
  );
...
function genExampleTask(tpl, task) {
  ...
  task_params {
    ${Object.keys(task)
      .map((key) => genExampleTaskParam(key, task[key]))
      .join("\n    ")}
  }
}`;
}
...
function genExampleTaskParam(key, val) {
  ...
  if (typeof val === "object") {
    return `${key} = {
      ${Object.keys(val)
        .map((k) => genExampleTaskParam(k, val[k]))
        .join("\n      ")}
    }`;
  }
}
