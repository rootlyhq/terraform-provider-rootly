// Code generated by rootly-providerctl. DO NOT EDIT.

package core

import (
	"context"

	"github.com/hashicorp/terraform-plugin-framework/datasource"
	datasourceschema "github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

var _ datasource.DataSource = &ServiceDataSource{}

type ServiceDataSource struct{}

func NewServiceDataSource() datasource.DataSource {
	return &ServiceDataSource{}
}

func (d *ServiceDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_service"
}

func (d *ServiceDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = datasourceschema.Schema{
		Attributes: map[string]datasourceschema.Attribute{
			"id": datasourceschema.StringAttribute{
				Computed: true,
			},
			"alert_broadcast_channel": datasourceschema.SingleNestedAttribute{
				Attributes: map[string]datasourceschema.Attribute{
					"id": datasourceschema.StringAttribute{
						MarkdownDescription: "Slack channel ID",
						Computed:            true,
					},
					"name": datasourceschema.StringAttribute{
						MarkdownDescription: "Slack channel name",
						Computed:            true,
					},
				},
				MarkdownDescription: "Slack channel to broadcast alerts to",
				Computed:            true,
			},
			"alert_broadcast_enabled": datasourceschema.BoolAttribute{
				MarkdownDescription: "Enable alerts to be broadcasted to a specific channel",
				Optional:            true,
				Computed:            true,
			},
			"alert_urgency_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The alert urgency id of the service",
				Computed:            true,
			},
			"alerts_email_address": datasourceschema.StringAttribute{
				MarkdownDescription: "Email generated to send alerts to",
				Computed:            true,
			},
			"alerts_email_enabled": datasourceschema.BoolAttribute{
				MarkdownDescription: "Enable alerts through email",
				Computed:            true,
			},
			"backstage_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The Backstage entity id associated to this service. eg: :namespace/:kind/:entity_name",
				Optional:            true,
				Computed:            true,
			},
			"color": datasourceschema.StringAttribute{
				MarkdownDescription: "The hex color of the service",
				Computed:            true,
			},
			"cortex_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The Cortex group id associated to this service",
				Optional:            true,
				Computed:            true,
			},
			"description": datasourceschema.StringAttribute{
				MarkdownDescription: "The description of the service",
				Computed:            true,
			},
			"environment_ids": datasourceschema.ListAttribute{
				ElementType:         types.StringType,
				MarkdownDescription: "Environments associated with this service",
				Computed:            true,
			},
			"escalation_policy_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The escalation policy id of the service",
				Computed:            true,
			},
			"external_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The external id associated to this service",
				Optional:            true,
				Computed:            true,
			},
			"github_repository_branch": datasourceschema.StringAttribute{
				MarkdownDescription: "The GitHub repository branch associated to this service. eg: main",
				Computed:            true,
			},
			"github_repository_name": datasourceschema.StringAttribute{
				MarkdownDescription: "The GitHub repository name associated to this service. eg: rootlyhq/my-service",
				Computed:            true,
			},
			"gitlab_repository_branch": datasourceschema.StringAttribute{
				MarkdownDescription: "The GitLab repository branch associated to this service. eg: main",
				Computed:            true,
			},
			"gitlab_repository_name": datasourceschema.StringAttribute{
				MarkdownDescription: "The GitLab repository name associated to this service. eg: rootlyhq/my-service",
				Computed:            true,
			},
			"incident_broadcast_channel": datasourceschema.SingleNestedAttribute{
				Attributes: map[string]datasourceschema.Attribute{
					"id": datasourceschema.StringAttribute{
						MarkdownDescription: "Slack channel ID",
						Computed:            true,
					},
					"name": datasourceschema.StringAttribute{
						MarkdownDescription: "Slack channel name",
						Computed:            true,
					},
				},
				MarkdownDescription: "Slack channel to broadcast incidents to",
				Computed:            true,
			},
			"incident_broadcast_enabled": datasourceschema.BoolAttribute{
				MarkdownDescription: "Enable incidents to be broadcasted to a specific channel",
				Optional:            true,
				Computed:            true,
			},
			"kubernetes_deployment_name": datasourceschema.StringAttribute{
				MarkdownDescription: "The Kubernetes deployment name associated to this service. eg: namespace/deployment-name",
				Computed:            true,
			},
			"name": datasourceschema.StringAttribute{
				MarkdownDescription: "The name of the service",
				Optional:            true,
				Computed:            true,
			},
			"notify_emails": datasourceschema.ListAttribute{
				ElementType:         types.StringType,
				MarkdownDescription: "Emails attached to the service",
				Computed:            true,
			},
			"opsgenie_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The Opsgenie service id associated to this service",
				Computed:            true,
			},
			"owner_group_ids": datasourceschema.ListAttribute{
				ElementType:         types.StringType,
				MarkdownDescription: "Owner Teams associated with this service",
				Computed:            true,
			},
			"owner_user_ids": datasourceschema.ListAttribute{
				ElementType:         types.Int64Type,
				MarkdownDescription: "Owner Users associated with this service",
				Computed:            true,
			},
			"pagerduty_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The PagerDuty service id associated to this service",
				Computed:            true,
			},
			"position": datasourceschema.Int64Attribute{
				MarkdownDescription: "Position of the service",
				Computed:            true,
			},
			"properties": datasourceschema.ListNestedAttribute{
				NestedObject: datasourceschema.NestedAttributeObject{
					Attributes: map[string]datasourceschema.Attribute{
						"catalog_property_id": datasourceschema.StringAttribute{
							MarkdownDescription: "Catalog property ID",
							Computed:            true,
						},
						"value": datasourceschema.StringAttribute{
							MarkdownDescription: "The property value",
							Computed:            true,
						},
					},
				},
				MarkdownDescription: "Array of property values for this service.",
				Computed:            true,
			},
			"public_description": datasourceschema.StringAttribute{
				MarkdownDescription: "The public description of the service",
				Computed:            true,
			},
			"service_ids": datasourceschema.ListAttribute{
				ElementType:         types.StringType,
				MarkdownDescription: "Services dependent on this service",
				Computed:            true,
			},
			"service_now_ci_sys_id": datasourceschema.StringAttribute{
				MarkdownDescription: "The Service Now CI sys id associated to this service",
				Computed:            true,
			},
			"slack_aliases": datasourceschema.ListNestedAttribute{
				NestedObject: datasourceschema.NestedAttributeObject{
					Attributes: map[string]datasourceschema.Attribute{
						"id": datasourceschema.StringAttribute{
							MarkdownDescription: "Slack alias ID",
							Computed:            true,
						},
						"name": datasourceschema.StringAttribute{
							MarkdownDescription: "Slack alias name",
							Computed:            true,
						},
					},
				},
				MarkdownDescription: "Slack Aliases associated with this service",
				Computed:            true,
			},
			"slack_channels": datasourceschema.ListNestedAttribute{
				NestedObject: datasourceschema.NestedAttributeObject{
					Attributes: map[string]datasourceschema.Attribute{
						"id": datasourceschema.StringAttribute{
							MarkdownDescription: "Slack channel ID",
							Computed:            true,
						},
						"name": datasourceschema.StringAttribute{
							MarkdownDescription: "Slack channel name",
							Computed:            true,
						},
					},
				},
				MarkdownDescription: "Slack Channels associated with this service",
				Computed:            true,
			},
			"slug": datasourceschema.StringAttribute{
				MarkdownDescription: "The slug of the service",
				Optional:            true,
				Computed:            true,
			},
		},
	}
}

func (d *ServiceDataSource) Read(_ context.Context, _ datasource.ReadRequest, _ *datasource.ReadResponse) {
}
