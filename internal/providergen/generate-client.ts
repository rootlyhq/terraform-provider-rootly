...
export function generateClient({
  ...
  let createSchema = removeReference(
    removeReference(
  ...
  if (!createSchema) {
    throw new Error(`Cannot find create schema: ${config.name} in doc`);
  }

  let updateSchema = removeReference(
    removeReference(
  ...
  if (!updateSchema) {
    throw new Error(`Cannot find update schema: ${config.name} in doc`);
  }
...
${generateModel({
  schema: createSchema,
  name: `Create${camelize(config.name)}`,
  parents: [],
})}

${generateModel({
  schema: updateSchema,
  name: `Update${camelize(config.name)}`,
  parents: [],
})}
...
${config.actions?.create?.enabled ? generateCreateAction({ doc, config }) : ""}

${config.actions?.update?.enabled ? generateUpdateAction({ doc, config }) : ""}
`;
}
...
function generateCreateAction({
  ...
  funcArgs.push(`req Create${camelize(config.name)}`);
...
function generateUpdateAction({
  ...
  funcArgs.push(`req Update${camelize(config.name)}`);
  clientArgs.push(`"application/vnd.api+json"`, "bytes.NewReader(buf.Bytes())");
