...
var _ validator.List = UniqueByAttributeValidator{}

type UniqueByAttributeValidator struct {
	AttributeName string
}

// UniqueByAttribute returns a list validator that checks that a given attribute
// is unique across all elements in the list.
func UniqueByAttribute(attributeName string) validator.List {
...
func (v UniqueByAttributeValidator) Description(ctx context.Context) string {
	return fmt.Sprintf("Ensures that all elements in the list have a unique value for %s.", v.AttributeName)
}
...
func (v UniqueByAttributeValidator) ValidateList(ctx context.Context, req validator.ListRequest, resp *validator.ListResponse) {
	...
	for index, elem := range elements {
		...
		// Exact path to the target attribute on this specific list element
		elemAttrPath := req.Path.AtListIndex(index).AtName(v.AttributeName)
			...
			resp.Diagnostics.AddAttributeError(
				...
				fmt.Sprintf(
					"Attribute %q has duplicate value %s.\n\n"+
						...
						"Values for %q must be unique across all elements in the list.",
					v.AttributeName,
					attrValue.String(),
