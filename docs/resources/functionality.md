...
- `slug` (String, Deprecated) The slug of the functionality

### Read-Only

- `id` (String) The ID of this resource.
- `managed_by` (String) How this functionality is managed (provenance): web, api, terraform, etc. Read-only.. Value must be one of `web`, `admin_web`, `api`, `terraform`, `pulumi`, `backstage`, `catalog_sync`.
...
rootly_functionality can be imported using the [`import` command](https://developer.hashicorp.com/terraform/cli/commands/import).

```sh
terraform import rootly_functionality.primary a816421c-6ceb-481a-87c4-585e47451f24
```
...
```terraform
import {
  to = rootly_functionality.primary
