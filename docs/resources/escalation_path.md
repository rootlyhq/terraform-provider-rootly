...
- `notification_type_rules` (Block List, Max: 10) Rules deciding whether an alert pages audible or quiet, evaluated in order — the first matching rule's notification_type wins, otherwise notification_type_fallback applies. When present, the path's notification_type is aligned to notification_type_fallback. Only available when notification type conditions are enabled for the team. Maximum of 10 rules. (see [below for nested schema](#nestedblock--notification_type_rules))
...
- `conditions` (Block List, Min: 1, Max: 5) Conditions combined per match_mode, at least one per rule. A deferral_window condition matches when the alert falls inside its time blocks. Maximum of 5 conditions. (see [below for nested schema](#nestedblock--notification_type_rules--conditions))
...
- `time_blocks` (Block List) Time windows during which alerts are deferred. Only used with `deferral_window` rule type. (see [below for nested schema](#nestedblock--notification_type_rules--conditions--time_blocks))
