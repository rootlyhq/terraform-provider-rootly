...
- `service_ids` (Set of String) Service ids for which this escalation path should be used. Only used with `service` rule type.
...
- `values` (Set of String) Values to match against. Used with `field` and `source` rule types.
