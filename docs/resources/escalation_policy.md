...
- `days` (Set of String) Business days. Value must be one of `M`, `T`, `W`, `R`, `F`, `U`, `S`.
...
rootly_escalation_policy can be imported using the [`import` command](https://developer.hashicorp.com/terraform/cli/commands/import).

```sh
terraform import rootly_escalation_policy.primary a816421c-6ceb-481a-87c4-585e47451f24
```
...
```terraform
import {
  to = rootly_escalation_policy.primary
