...
- `repository` (Map of String) Map must contain two fields, `id` and `name`. The repository (used for loading labels and issue types)
- `task_type` (String)
...
rootly_workflow_task_update_github_issue can be imported using the [`import` command](https://developer.hashicorp.com/terraform/cli/commands/import).

```sh
terraform import rootly_workflow_task_update_github_issue.primary a816421c-6ceb-481a-87c4-585e47451f24
```
...
```terraform
import {
  to = rootly_workflow_task_update_github_issue.primary
